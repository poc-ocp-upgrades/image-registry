...
func TestManifest(t *testing.T) {
	...
	logger := logger.New()
	driver := &storageDriver{storageDriver: inmemory.New(), logger: logger}
	...
	if err := logger.Compare([]string{"GET " + storagepath.Manifest(reponame, dgst), "GET " + storagepath.Layer(reponame, dgst)}); err != nil {
		t.Fatal(err)
	}
}
