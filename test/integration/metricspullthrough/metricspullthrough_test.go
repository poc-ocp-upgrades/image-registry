...
func TestPullthroughBlob(t *testing.T) {
	...
	ts := testframework.NewHTTPServer(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if testframework.ServeV2(w, r) || testframework.ServeImage(w, r, "remoteimage", imageData, []string{"latest"}) {
			return
		}
	...
	metrics := []struct {
		name	string
		values	[]string
	}{{name: "imageregistry_storage_duration_seconds", values: []string{`operation="StorageDriver.Stat"`}}, {name: "imageregistry_storage_errors_total", values: []string{`operation="StorageDriver.Stat"`, `code="PATH_NOT_FOUND"`}}, {name: "imageregistry_pullthrough_repository_errors_total", values: []string{`operation="BlobStore.Stat"`, `code="UNAUTHORIZED"`}}, {name: "imageregistry_pullthrough_blobstore_cache_requests_total", values: []string{`type="Miss"`}}, {name: "imageregistry_pullthrough_repository_duration_seconds", values: []string{`operation="Init"`}}}
