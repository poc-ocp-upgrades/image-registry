...
func signedManifest(name string, blobs []digest.Digest) ([]byte, digest.Digest, error) {
	...
	mappingManifest := schema1.Manifest{Versioned: manifest.Versioned{SchemaVersion: 1}, Name: name, Tag: imageapi.DefaultImageTag, Architecture: "amd64", History: history, FSLayers: fsLayers}
...
func TestV2RegistryGetTags(t *testing.T) {
	...
	stream := imageapiv1.ImageStream{ObjectMeta: metav1.ObjectMeta{Namespace: namespace, Name: "test"}}
