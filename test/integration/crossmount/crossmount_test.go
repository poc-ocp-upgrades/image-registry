...
func crossMountImage(ctx context.Context, destRepo distribution.Repository, tag string, srcRepoNamed reference.Named, manifest distribution.Manifest) error {
	...
	for _, desc := range manifest.References() {
		...
		bw.Cancel(ctx)
		bw.Close()
		return errRegistryWantsContent{src: canonicalRef, dst: destRepo.Named()}
	}
...
func TestCrossMount(t *testing.T) {
	...
	}{{name: "mount own image into the same namespace", actor: alice, source: uploadedImage(alice, "foo"), destination: sameProject("mounted-foo"), check: wantSuccess}, {name: "mount own copied image into the same namespace", actor: alice, source: copiedImage(alice, "foo", "foo-copy"), destination: sameProject("mounted-foo-copy"), check: wantSuccess}, {name: "mount another's image", actor: bob, source: uploadedImage(alice, "foo"), destination: anotherProject(bob, "from-foo"), check: wantCrossMountError}, {name: "mount another's copied image", actor: bob, source: copiedImage(alice, "foo", "foo-copy"), destination: anotherProject(bob, "from-foo-copy"), check: wantCrossMountError}} {
