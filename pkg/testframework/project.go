...
func CreateProject(t *testing.T, clientConfig *rest.Config, namespace string, adminUser string) *projectapiv1.Project {
	...
	_, err = authorizationClient.RoleBindings(namespace).Update(&authorizationapiv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: "admin"}, UserNames: []string{adminUser}, RoleRef: corev1.ObjectReference{Name: "admin"}})
	if err != nil {
		t.Fatal(err)
	}
