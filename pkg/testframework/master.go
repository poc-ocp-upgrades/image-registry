...
type MasterProcess struct{ kubeconfig string }
...
func StartMasterProcess(kubeconfig string) (MasterInterface, error) {
	...
	return &MasterProcess{kubeconfig: kubeconfig}, nil
}
...
func (p *MasterProcess) WaitHealthz(configDir string) error {
	...
	rt := &http.Transport{Proxy: http.ProxyFromEnvironment, DialContext: (&net.Dialer{Timeout: 30 * time.Second, KeepAlive: 30 * time.Second, DualStack: true}).DialContext, MaxIdleConns: 100, IdleConnTimeout: 90 * time.Second, TLSHandshakeTimeout: 10 * time.Second, ExpectContinueTimeout: 1 * time.Second, TLSClientConfig: tlsConfig}
...
type MasterContainer struct {
	ID				string
	Port			int
	NetworkSettings	struct{ IPAddress string }
...
func StartMasterContainer(configDir string) (MasterInterface, error) {
	...
	resp, err := cli.ContainerCreate(ctx, &container.Config{Image: originImageRef, Entrypoint: []string{"/bin/sh"}, Cmd: []string{"-ec", `
	...
	c := &MasterContainer{ID: resp.ID, Port: 8443, KubeConfigPath: filepath.Join(configDir, "master", "admin.kubeconfig")}
...
func (c *MasterContainer) WriteConfigs(configDir string) error {
	...
	srcInfo := archive.CopyInfo{Path: srcPath, Exists: true, IsDir: stat.Mode.IsDir()}
...
func NewMaster(t *testing.T) *Master {
	...
	m := &Master{t: t, tmpDir: tmpDir, container: container}
...
func (m *Master) CreateUser(username string, password string) *User {
	...
	return &User{Name: username, Token: user.BearerToken, kubeConfig: UserClientConfig(m.AdminKubeConfig(), user.BearerToken)}
}
