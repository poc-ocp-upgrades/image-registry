...
func ExecuteRestore(configFile io.Reader, mode, namespace string) {
	...
	if strings.HasPrefix(mode, "check") {
		...
		restore = &prune.StorageRestore{Ctx: ctx, Client: client}
	}
	fsck := &prune.Fsck{Ctx: ctx, Client: client, Registry: registry, ServerAddr: config.Server.Addr, Restore: restore}
	switch mode {
