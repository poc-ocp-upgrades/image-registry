...
func TestUnmarshalManifestSchema2(t *testing.T) {
	...
	}{{name: "valid nginx image with sizes in manifest", manifestString: manifestSchema2, expectedConfig: manifestSchema2ConfigDescriptor, expectedReferences: []distribution.Descriptor{manifestSchema2ConfigDescriptor, manifestSchema2LayerDescriptors[0], manifestSchema2LayerDescriptors[1], manifestSchema2LayerDescriptors[2]}}, {name: "invalid schema2 image", manifestString: manifestSchema2Invalid, expectedErrorSubstring: "invalid character"}, {name: "manifest schema1 image", manifestString: manifestSchema1, expectedErrorSubstring: "mediaType in manifest should be 'application/vnd.docker.distribution.manifest.v2+json'"}} {
