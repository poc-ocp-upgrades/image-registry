...
func TestPullthroughServeBlob(t *testing.T) {
	...
	testutil.AddImageStream(t, fos, namespace, name, map[string]string{imageapi.InsecureRepositoryAnnotation: "true"})
	...
	for _, tc := range []struct {
	...
	}{{name: "stat local blob", method: "HEAD", blobDigest: blob1Desc.Digest, localBlobs: blob1Storage, expectedContentLength: int64(len(blob1Content)), expectedLocalCalls: map[string]int{"Stat": 1, "ServeBlob": 1}}, {name: "serve local blob", method: "GET", blobDigest: blob1Desc.Digest, localBlobs: blob1Storage, expectedContentLength: int64(len(blob1Content)), expectedBytesServed: int64(len(blob1Content)), expectedBytesServedLocally: int64(len(blob1Content)), expectedLocalCalls: map[string]int{"Stat": 1, "ServeBlob": 1}}, {name: "stat remote blob", method: "HEAD", blobDigest: blob1Desc.Digest, localBlobs: blob2Storage, expectedContentLength: int64(len(blob1Content)), expectedLocalCalls: map[string]int{"Stat": 1, "ServeBlob": 1}}, {name: "serve remote blob", method: "GET", blobDigest: blob1Desc.Digest, expectedContentLength: int64(len(blob1Content)), expectedBytesServed: int64(len(blob1Content)), expectedLocalCalls: map[string]int{"Stat": 1, "ServeBlob": 1}}, {name: "unknown blob digest", method: "GET", blobDigest: unknownBlobDigest, expectedStatError: distribution.ErrBlobUnknown, expectedLocalCalls: map[string]int{"Stat": 1}}} {
		...
		digestCache, err := cache.NewBlobDigest(defaultDescriptorCacheSize, defaultDigestToRepositoryCacheSize, 24*time.Hour, metrics.NewNoopMetrics())
		if err != nil {
			t.Fatalf("unable to create cache: %v", err)
		}
		cache := cache.NewRepositoryDigest(digestCache)
		remoteBlobGetter := NewBlobGetterService(imageStream, imageStream.GetSecrets, cache, metrics.NewNoopMetrics())
		ptbs := &pullthroughBlobStore{BlobStore: localBlobStore, remoteBlobGetter: remoteBlobGetter}
...
func TestPullthroughServeNotSeekableBlob(t *testing.T) {
	...
	ptbs := &pullthroughBlobStore{BlobStore: newTestBlobStore(nil, nil), remoteBlobGetter: repoBlobs, mirror: false}
...
func TestPullthroughServeBlobInsecure(t *testing.T) {
	...
	m1dgst, m1canonical, m1cfg, m1manifest, err := testutil.CreateAndUploadTestManifest(ctx, testutil.ManifestSchema2, 2, serverURL, nil, repo1Name, "foo")
	if err != nil {
	...
	m2dgst, m2canonical, m2cfg, m2manifest, err := testutil.CreateAndUploadTestManifest(ctx, testutil.ManifestSchema2, 2, serverURL, nil, repo2Name, "bar")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	for _, tc := range []struct {
		...
		expectedBytesServedLocally	int64
		expectedLocalCalls		map[string]int
	}{{name: "stat remote blob with insecure repository", method: "HEAD", blobDigest: digest.Digest(m1img.DockerImageLayers[0].Name), fakeOpenShiftInit: func(fos *testutil.FakeOpenShift) {
		testutil.AddImageStream(t, fos, namespace, repo1, map[string]string{imageapi.InsecureRepositoryAnnotation: "true"})
		testutil.AddImage(t, fos, m1img, namespace, repo1, "tag1")
	}, expectedContentLength: int64(m1img.DockerImageLayers[0].LayerSize), expectedLocalCalls: map[string]int{"Stat": 1, "ServeBlob": 1}}, {name: "serve remote blob with insecure repository", method: "GET", blobDigest: digest.Digest(m1img.DockerImageLayers[0].Name), fakeOpenShiftInit: func(fos *testutil.FakeOpenShift) {
		testutil.AddImageStream(t, fos, namespace, repo1, map[string]string{imageapi.InsecureRepositoryAnnotation: "true"})
		testutil.AddImage(t, fos, m1img, namespace, repo1, "tag1")
	}, expectedContentLength: int64(m1img.DockerImageLayers[0].LayerSize), expectedBytesServed: int64(m1img.DockerImageLayers[0].LayerSize), expectedLocalCalls: map[string]int{"Stat": 1, "ServeBlob": 1}}, {name: "stat remote blob with secure repository", method: "HEAD", blobDigest: digest.Digest(m1img.DockerImageLayers[0].Name), fakeOpenShiftInit: func(fos *testutil.FakeOpenShift) {
		testutil.AddImageStream(t, fos, namespace, repo1, map[string]string{imageapi.InsecureRepositoryAnnotation: "false"})
		testutil.AddImage(t, fos, m1img, namespace, repo1, "tag1")
	}, expectedStatError: distribution.ErrBlobUnknown}, {name: "serve remote blob with secure repository", method: "GET", blobDigest: digest.Digest(m1img.DockerImageLayers[0].Name), fakeOpenShiftInit: func(fos *testutil.FakeOpenShift) {
		testutil.AddImageStream(t, fos, namespace, repo1, map[string]string{imageapi.InsecureRepositoryAnnotation: "false"})
		testutil.AddImage(t, fos, m1img, namespace, repo1, "tag1")
	}, expectedStatError: distribution.ErrBlobUnknown}, {name: "stat remote blob with with insecure tag", method: "HEAD", blobDigest: digest.Digest(m2img.DockerImageLayers[0].Name), fakeOpenShiftInit: func(fos *testutil.FakeOpenShift) {
		testutil.AddUntaggedImage(t, fos, m1img)
		testutil.AddUntaggedImage(t, fos, m2img)
		testutil.AddImageStream(t, fos, namespace, repo1, map[string]string{imageapi.InsecureRepositoryAnnotation: "false"})
		testutil.AddImageStreamTag(t, fos, m1img, namespace, repo1, &imageapiv1.TagReference{Name: "tag1", ImportPolicy: imageapiv1.TagImportPolicy{Insecure: false}})
		testutil.AddImageStreamTag(t, fos, m2img, namespace, repo1, &imageapiv1.TagReference{Name: "tag2", ImportPolicy: imageapiv1.TagImportPolicy{Insecure: true}})
	}, expectedContentLength: int64(m2img.DockerImageLayers[0].LayerSize), expectedLocalCalls: map[string]int{"Stat": 1, "ServeBlob": 1}}, {name: "serve remote blob with insecure tag", method: "GET", blobDigest: digest.Digest(m2img.DockerImageLayers[0].Name), fakeOpenShiftInit: func(fos *testutil.FakeOpenShift) {
		testutil.AddUntaggedImage(t, fos, m1img)
		testutil.AddUntaggedImage(t, fos, m2img)
		testutil.AddImageStream(t, fos, namespace, repo1, map[string]string{imageapi.InsecureRepositoryAnnotation: "false"})
		testutil.AddImageStreamTag(t, fos, m1img, namespace, repo1, &imageapiv1.TagReference{Name: "tag1", ImportPolicy: imageapiv1.TagImportPolicy{Insecure: false}})
		testutil.AddImageStreamTag(t, fos, m2img, namespace, repo1, &imageapiv1.TagReference{Name: "tag2", ImportPolicy: imageapiv1.TagImportPolicy{Insecure: true}})
	}, expectedLocalCalls: map[string]int{"Stat": 1, "ServeBlob": 1}, expectedContentLength: int64(m2img.DockerImageLayers[0].LayerSize), expectedBytesServed: int64(m2img.DockerImageLayers[0].LayerSize)}, {name: "insecure flag propagates to all repositories of the registry", method: "GET", blobDigest: digest.Digest(m2img.DockerImageLayers[0].Name), fakeOpenShiftInit: func(fos *testutil.FakeOpenShift) {
		testutil.AddUntaggedImage(t, fos, m1img)
		testutil.AddUntaggedImage(t, fos, m2img)
		testutil.AddImageStream(t, fos, namespace, repo1, map[string]string{imageapi.InsecureRepositoryAnnotation: "false"})
		testutil.AddImageStreamTag(t, fos, m1img, namespace, repo1, &imageapiv1.TagReference{Name: "tag1", ImportPolicy: imageapiv1.TagImportPolicy{Insecure: true}})
		testutil.AddImageStreamTag(t, fos, m2img, namespace, repo1, &imageapiv1.TagReference{Name: "tag2", ImportPolicy: imageapiv1.TagImportPolicy{}})
	}, expectedLocalCalls: map[string]int{"Stat": 1, "ServeBlob": 1}, expectedContentLength: int64(m2img.DockerImageLayers[0].LayerSize), expectedBytesServed: int64(m2img.DockerImageLayers[0].LayerSize)}, {name: "serve remote blob with secure tag", method: "GET", blobDigest: digest.Digest(m1img.DockerImageLayers[0].Name), fakeOpenShiftInit: func(fos *testutil.FakeOpenShift) {
		testutil.AddUntaggedImage(t, fos, m1img)
		testutil.AddUntaggedImage(t, fos, m2img)
		m2docker := *m2img
		...
		testutil.AddImageStream(t, fos, namespace, repo1, map[string]string{imageapi.InsecureRepositoryAnnotation: "false"})
		testutil.AddImageStreamTag(t, fos, m1img, namespace, repo1, &imageapiv1.TagReference{Name: "tag1", ImportPolicy: imageapiv1.TagImportPolicy{Insecure: false}})
		testutil.AddImageStreamTag(t, fos, &m2docker, namespace, repo1, &imageapiv1.TagReference{Name: "tag2", ImportPolicy: imageapiv1.TagImportPolicy{Insecure: true}})
	}, expectedStatError: distribution.ErrBlobUnknown}, {name: "serve remote blob with 2 tags pointing to the same image", method: "GET", blobDigest: digest.Digest(m1img.DockerImageLayers[0].Name), fakeOpenShiftInit: func(fos *testutil.FakeOpenShift) {
		testutil.AddUntaggedImage(t, fos, m1img)
		testutil.AddImageStream(t, fos, namespace, repo1, map[string]string{imageapi.InsecureRepositoryAnnotation: "false"})
		testutil.AddImageStreamTag(t, fos, m1img, namespace, repo1, &imageapiv1.TagReference{Name: "tag1", ImportPolicy: imageapiv1.TagImportPolicy{Insecure: false}})
		testutil.AddImageStreamTag(t, fos, m1img, namespace, repo1, &imageapiv1.TagReference{Name: "tag2", ImportPolicy: imageapiv1.TagImportPolicy{Insecure: true}})
	}, expectedLocalCalls: map[string]int{"Stat": 1, "ServeBlob": 1}, expectedContentLength: int64(m1img.DockerImageLayers[0].LayerSize), expectedBytesServed: int64(m1img.DockerImageLayers[0].LayerSize)}} {
		tc := tc
...
func TestPullthroughMetrics(t *testing.T) {
	...
	remoteBlobGetter := NewBlobGetterService(imageStream, imageStream.GetSecrets, cache, metrics.NewMetrics(sink))
	ptbs := &pullthroughBlobStore{BlobStore: localBlobStore, remoteBlobGetter: remoteBlobGetter}
	...
	if diff := c.Diff(counter.M{"pullthrough_blobstore_cache_requests:Miss": 1, fmt.Sprintf("pullthrough_repository:%s:Init", serverURL.Host): 1, fmt.Sprintf("pullthrough_repository:%s:BlobStore.Stat", serverURL.Host): 1}); diff != nil {
		t.Fatal(diff)
	}
	_, err = ptbs.Stat(ctx, dgst)
	if err != nil {
	...
	if diff := c.Diff(counter.M{"pullthrough_blobstore_cache_requests:Miss": 1, "pullthrough_blobstore_cache_requests:Hit": 1, fmt.Sprintf("pullthrough_repository:%s:Init", serverURL.Host): 1, fmt.Sprintf("pullthrough_repository:%s:BlobStore.Stat", serverURL.Host): 2}); diff != nil {
		t.Fatal(diff)
	}
}
...
func (t *testBlobStore) Stat(ctx context.Context, dgst digest.Digest) (distribution.Descriptor, error) {
	...
	return distribution.Descriptor{MediaType: schema1.MediaTypeManifestLayer, Size: int64(len(content)), Digest: makeDigestFromBytes(content)}, nil
}
...
func (t *testBlobStore) Open(ctx context.Context, dgst digest.Digest) (distribution.ReadSeekCloser, error) {
	...
	return &testBlobFileReader{bs: t, content: content}, nil
}
