...
func TestManifestServiceExists(t *testing.T) {
	...
	ms := &manifestService{imageStream: imageStream, acceptSchema2: true}
...
func TestManifestServicePut(t *testing.T) {
	...
	bs := newTestBlobStore(nil, blobContents{"test:1": []byte("{}")})
	tms := newTestManifestService(repoName, nil)
	imageStream := imagestream.New(ctx, namespace, repo, registryclient.NewFakeRegistryAPIClient(nil, imageClient))
	ms := &manifestService{manifests: tms, blobStore: bs, imageStream: imageStream, acceptSchema2: true}
	manifest := &schema2.DeserializedManifest{Manifest: schema2.Manifest{Config: distribution.Descriptor{Digest: "test:1", Size: 2}}}
	...
	imageStream = imagestream.New(ctx, namespace, repo, registryclient.NewFakeRegistryAPIClient(nil, imageClient))
	ms = &manifestService{manifests: tms, imageStream: imageStream, acceptSchema2: true}
