...
func TestGlobalProviderStat(t *testing.T) {
	...
	cacheprovider := &Provider{Cache: cache}
	...
	err = cacheprovider.SetDescriptor(context.Background(), dgst, distribution.Descriptor{Digest: dgst, Size: 2})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	desc, err = cacheprovider.Stat(context.Background(), dgst)
...
func TestRepositoryScopedProviderStat(t *testing.T) {
	...
	dgst := digest.Digest("sha256:4355a46b19d348dc2f57c046f8ef63d4538ebb936000f3c9ee954a27460dd865")
	err = repoprovider.SetDescriptor(context.Background(), dgst, distribution.Descriptor{Digest: dgst, Size: 2})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	desc, err := cacheprovider.Stat(context.Background(), dgst)
