...
func TestRepositoryScopedBlobDescriptor(t *testing.T) {
	...
	svc := &fakeBlobDescriptorService{digests: map[digest.Digest]distribution.Descriptor{dgst: {Digest: dgst, Size: 2}}}
	bs := &RepositoryScopedBlobDescriptor{Repo: repo, Cache: cache, Svc: svc}
...
func TestRepositoryScopedBlobDescriptorClear(t *testing.T) {
	...
	svc := &fakeBlobDescriptorService{digests: map[digest.Digest]distribution.Descriptor{dgst: {Digest: dgst, Size: 2}}}
	bs := &RepositoryScopedBlobDescriptor{Cache: cache, Svc: svc}
	_, err = bs.Stat(context.Background(), dgst)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRepositoryScopedBlobDescriptorAdd(t *testing.T) {
	...
	err = bs.SetDescriptor(context.Background(), dgst, distribution.Descriptor{Digest: dgst, Size: 2})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if _, ok := svc.digests[dgst]; !ok {
