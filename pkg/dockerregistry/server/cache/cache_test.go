...
func TestDigestCacheAddDigest(t *testing.T) {
	...
	cache.Add(dgst, &DigestValue{desc: &distribution.Descriptor{Digest: dgst, Size: 1234}, repo: &repo})
...
func TestDigestCacheRemoveDigest(t *testing.T) {
	...
	err = cache.Add(dgst, &DigestValue{desc: &desc})
	if err != nil {
		t.Fatal(err)
	}
	_, err = cache.Get(dgst)
...
func TestDigestCacheAddRepository(t *testing.T) {
	...
	for _, v := range repos {
		cache.Add(dgst, &DigestValue{repo: &v})
	}
...
func TestDigestCacheDigestMigration(t *testing.T) {
	...
	cache.Add(dgst256, &DigestValue{desc: &distribution.Descriptor{Digest: dgst512, Size: 1234}})
