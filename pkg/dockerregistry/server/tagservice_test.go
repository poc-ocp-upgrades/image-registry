...
func TestTagGet(t *testing.T) {
	...
	}{{title: "get valid tag from managed image", tagName: tag, tagValue: distribution.Descriptor{Digest: digest.Digest(testImage.Name)}}, {title: "get missing tag", tagName: tag + "-no-found", expectedError: true, expectedNotFoundError: true}}
	for _, tc := range testcases {
		imageStream := imagestream.New(backgroundCtx, namespace, repo, registryclient.NewFakeRegistryAPIClient(nil, imageClient))
		ts := &tagService{TagService: newTestTagService(nil), imageStream: imageStream}
...
func TestTagGetAll(t *testing.T) {
	...
	testcases := []struct {
		...
		expectResult	[]string
		expectedError	bool
	}{{title: "get all tags", expectResult: []string{tag}}}
	for _, tc := range testcases {
		imageStream := imagestream.New(ctx, namespace, repo, registryclient.NewFakeRegistryAPIClient(nil, imageClient))
		ts := &tagService{TagService: newTestTagService(nil), imageStream: imageStream}
...
func TestTagLookup(t *testing.T) {
	...
	testcases := []struct {
		...
		expectResult	[]string
		expectedError	bool
	}{{title: "lookup tags with pullthrough", tagValue: distribution.Descriptor{Digest: digest.Digest(testImage.Name)}, expectResult: []string{tag}}, {title: "lookup tags by missing digest", tagValue: distribution.Descriptor{Digest: digest.Digest(etcdDigest)}, expectResult: []string{}}}
	for _, tc := range testcases {
		imageStream := imagestream.New(ctx, namespace, repo, registryclient.NewFakeRegistryAPIClient(nil, imageClient))
		ts := &tagService{TagService: newTestTagService(nil), imageStream: imageStream}
...
func TestTagLookupWithoutImageStream(t *testing.T) {
	...
	imageStream := imagestream.New(ctx, namespace, repo, registryclient.NewFakeRegistryAPIClient(nil, imageClient))
	ts := &tagService{TagService: newTestTagService(nil), imageStream: imageStream}
	_, err := ts.Lookup(ctx, distribution.Descriptor{Digest: digest.Digest(anotherImage.Name)})
	if err == nil {
		t.Fatalf("error expected")
	}
	_, ok := err.(distribution.ErrRepositoryUnknown)
