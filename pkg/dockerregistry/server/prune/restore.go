...
func (r *Fsck) checkImage(image *imageapiv1.Image, blobStatter *statter) error {
	...
	if err != nil {
		...
		return &ErrImage{Digest: imageDigest, Problem: distribution.ErrBlobUnknown}
	}
	if image.DockerImageManifestMediaType == schema2.MediaTypeManifest {
		...
		if ok {
			...
			if err != nil {
				...
				return &ErrImage{Digest: configDigest, Problem: distribution.ErrBlobUnknown}
			}
		}
	}
	for _, layer := range image.DockerImageLayers {
		...
		if err != nil {
			...
			return &ErrImage{Digest: layerDigest, Problem: distribution.ErrBlobUnknown}
		}
	}
	return nil
}
func (r *Fsck) Database(namespace string) error {
	...
	for _, is := range listIS.Items {
		stat := &statter{statter: r.Registry.BlobStatter()}
			...
			for i, tagEvent := range tagEventList.Items {
				...
				if err == nil {
					...
					case kerrors.IsNotFound(err):
						...
						continue
					case err != nil:
						return &ErrImage{Digest: imageDigest, Problem: err}
					}
...
func (r *Fsck) Storage(namespace string) error {
	...
	err := enumStorage.Repositories(r.Ctx, func(repoName string) error {
		...
		err = enumStorage.Manifests(r.Ctx, repoName, func(dgst digest.Digest) error {
			...
			image := &imageapiv1.Image{ObjectMeta: metav1.ObjectMeta{Name: dgst.String(), Annotations: map[string]string{imageapi.ManagedByOpenShiftAnnotation: "true", imageapi.ImageManifestBlobStoredAnnotation: "true", imageapi.DockerImageLayersOrderAnnotation: layerOrder}}, DockerImageReference: fmt.Sprintf("%s/%s@%s", r.ServerAddr, repoName, dgst.String()), DockerImageManifest: string(payload), DockerImageManifestMediaType: mediaType, DockerImageConfig: string(config), DockerImageLayers: layers}
