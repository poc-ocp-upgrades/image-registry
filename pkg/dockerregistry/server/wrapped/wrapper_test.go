...
func TestWrapper(t *testing.T) {
	...
	objects := []reflect.Value{reflect.ValueOf(&blobStore{wrapper: captureFuncName}), reflect.ValueOf(&manifestService{wrapper: captureFuncName}), reflect.ValueOf(&tagService{wrapper: captureFuncName}), reflect.ValueOf(&blobWriter{wrapper: captureFuncName}), reflect.ValueOf(&blobDescriptorService{wrapper: captureFuncName}), reflect.ValueOf(&storageDriver{wrapper: simplify(captureFuncName)}), reflect.ValueOf(&readCloser{wrapper: simplify(captureFuncName)}), reflect.ValueOf(&fileWriter{wrapper: simplify(captureFuncName)})}
	...
	objects = []reflect.Value{reflect.ValueOf(NewBlobStore(NewBlobStore(nil, dummyWrapper), checkWrapper)), reflect.ValueOf(NewManifestService(NewManifestService(nil, dummyWrapper), checkWrapper)), reflect.ValueOf(NewTagService(NewTagService(nil, dummyWrapper), checkWrapper)), reflect.ValueOf(NewBlobWriter(NewBlobWriter(nil, dummyWrapper), checkWrapper)), reflect.ValueOf(NewBlobDescriptorService(NewBlobDescriptorService(nil, dummyWrapper), checkWrapper)), reflect.ValueOf(NewStorageDriver(NewStorageDriver(nil, simplify(dummyWrapper)), simplify(checkWrapper))), reflect.ValueOf(NewReadCloser(NewReadCloser(nil, simplify(dummyWrapper)), simplify(checkWrapper))), reflect.ValueOf(NewFileWriter(NewFileWriter(nil, simplify(dummyWrapper)), simplify(checkWrapper)))}
	for _, v := range objects {
		typeName := strings.Title(v.Elem().Type().Name())
		for i := 0; i < v.Type().NumMethod(); i++ {
			lastResult.c = 0
