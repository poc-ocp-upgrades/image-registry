...
type testRegistryClient struct{ client client.Interface }
...
func newTestRegistry(ctx context.Context, osClient registryclient.Interface, storageDriver driver.StorageDriver, blobrepositorycachettl time.Duration, useBlobDescriptorCacheProvider bool) (distribution.Namespace, error) {
	...
	digestCache, err := cache.NewBlobDigest(defaultDescriptorCacheSize, defaultDigestToRepositoryCacheSize, cfg.Cache.BlobRepositoryTTL, metrics.NewNoopMetrics())
	if err != nil {
		return nil, err
	}
	app := &App{registryClient: &testRegistryClient{client: osClient}, config: cfg, cache: digestCache, quotaEnforcing: &quotaEnforcingConfig{enforcementEnabled: false}, metrics: metrics.NewNoopMetrics(), paginationCache: kubecache.NewLRUExpireCache(128)}
	if storageDriver == nil {
		storageDriver = inmemory.New()
	}
	opts := []storage.RegistryOption{storage.EnableDelete, storage.EnableRedirect}
