...
func TestPullthroughManifests(t *testing.T) {
	...
	ms1dgst, ms1canonical, _, ms1manifest, err := testutil.CreateAndUploadTestManifest(ctx, testutil.ManifestSchema1, 2, serverURL, nil, repoName, "schema1")
	if err != nil {
	...
	ms2dgst, ms2canonical, _, ms2manifest, err := testutil.CreateAndUploadTestManifest(ctx, testutil.ManifestSchema1, 2, serverURL, nil, repoName, "schema1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	testutil.AddImageStream(t, fos, namespace, repo, map[string]string{imageapi.InsecureRepositoryAnnotation: "true"})
	...
	for _, tc := range []struct {
	...
	}{{name: "manifest digest from local store", manifestDigest: ms1dgst, localData: map[digest.Digest]distribution.Manifest{ms1dgst: ms1manifest}, expectedLocalCalls: map[string]int{"Get": 1}}, {name: "manifest served from remote repository", manifestDigest: ms1dgst, expectedLocalCalls: map[string]int{"Get": 1}}, {name: "unknown manifest digest", manifestDigest: unknownBlobDigest, expectedNotFoundError: true, expectedLocalCalls: map[string]int{"Get": 1}}, {name: "skip pullthrough for internal image manifest digest", manifestDigest: ms2dgst, expectedNotFoundError: true, expectedLocalCalls: map[string]int{"Get": 1}}} {
		...
		ptms := &pullthroughManifestService{ManifestService: localManifestService, imageStream: imageStream, cache: cache, registryAddr: "localhost:5000", metrics: metrics.NewNoopMetrics()}
...
func TestPullthroughManifestInsecure(t *testing.T) {
	...
	t.Logf("ms1dgst=%s, ms1manifest: %s", ms1dgst, ms1canonical)
	ms2dgst, ms2canonical, ms2config, ms2manifest, err := testutil.CreateAndUploadTestManifest(ctx, testutil.ManifestSchema2, 2, serverURL, nil, repoName, "schema2")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	_, ms2payload, err := ms2manifest.Payload()
	...
	for _, tc := range []struct {
	...
	}{{name: "fetch schema 1 with allowed insecure", manifestDigest: ms1dgst, fakeOpenShiftInit: func(fos *testutil.FakeOpenShift) {
		testutil.AddImageStream(t, fos, namespace, repo, map[string]string{imageapi.InsecureRepositoryAnnotation: "true"})
		testutil.AddImage(t, fos, ms1img, namespace, repo, "schema1")
	}, expectedManifest: ms1manifest, expectedLocalCalls: map[string]int{"Get": 1}}, {name: "fetch schema 2 with allowed insecure", manifestDigest: ms2dgst, fakeOpenShiftInit: func(fos *testutil.FakeOpenShift) {
		testutil.AddImageStream(t, fos, namespace, repo, map[string]string{imageapi.InsecureRepositoryAnnotation: "true"})
		testutil.AddImage(t, fos, ms2img, namespace, repo, "schema2")
	}, expectedManifest: ms2manifest, expectedLocalCalls: map[string]int{"Get": 1}}, {name: "explicit forbid insecure", manifestDigest: ms1dgst, fakeOpenShiftInit: func(fos *testutil.FakeOpenShift) {
		testutil.AddImageStream(t, fos, namespace, repo, map[string]string{imageapi.InsecureRepositoryAnnotation: "false"})
		testutil.AddImage(t, fos, ms1img, namespace, repo, "schema1")
	}, expectedErrorString: "server gave HTTP response to HTTPS client", expectedLocalCalls: map[string]int{"Get": 1}}, {name: "implicit forbid insecure", manifestDigest: ms1dgst, fakeOpenShiftInit: func(fos *testutil.FakeOpenShift) {
		testutil.AddImageStream(t, fos, namespace, repo, nil)
		testutil.AddImage(t, fos, ms1img, namespace, repo, "schema1")
	}, expectedErrorString: "server gave HTTP response to HTTPS client", expectedLocalCalls: map[string]int{"Get": 1}}, {name: "pullthrough from insecure tag", manifestDigest: ms1dgst, fakeOpenShiftInit: func(fos *testutil.FakeOpenShift) {
		image, err := testutil.NewImageForManifest(repoName, string(ms1payload), "", false)
		if err != nil {
			t.Fatal(err)
		}
		...
		testutil.AddImageStream(t, fos, namespace, repo, nil)
		testutil.AddImageStreamTag(t, fos, ms1img, namespace, repo, &imageapiv1.TagReference{Name: "schema1", ImportPolicy: imageapiv1.TagImportPolicy{Insecure: true}})
	}, expectedManifest: ms1manifest, expectedLocalCalls: map[string]int{"Get": 1}}, {name: "pull insecure if either image stream is insecure or the tag", manifestDigest: ms2dgst, fakeOpenShiftInit: func(fos *testutil.FakeOpenShift) {
		...
		testutil.AddUntaggedImage(t, fos, image)
		testutil.AddImageStream(t, fos, namespace, repo, map[string]string{imageapi.InsecureRepositoryAnnotation: "true"})
		testutil.AddImageStreamTag(t, fos, image, namespace, repo, &imageapiv1.TagReference{Name: "schema2", ImportPolicy: imageapiv1.TagImportPolicy{Insecure: false}})
	}, expectedManifest: ms2manifest, expectedLocalCalls: map[string]int{"Get": 1}}} {
		t.Run(tc.name, func(t *testing.T) {
			...
			cache := cache.NewRepositoryDigest(digestCache)
			ptms := &pullthroughManifestService{ManifestService: localManifestService, imageStream: imageStream, cache: cache, metrics: metrics.NewNoopMetrics()}
			manifestResult, err := ptms.Get(ctx, tc.manifestDigest)
			switch err.(type) {
			case nil:
...
func TestPullthroughManifestDockerReference(t *testing.T) {
	...
	testutil.AddImageStream(t, fos, namespace, repo2, map[string]string{imageapi.InsecureRepositoryAnnotation: "true"})
	...
	for _, tc := range []struct {
	...
	}{{name: "server 1", repoName: repo1, touchedServers: []*testServer{server1}, untouchedServers: []*testServer{server2}}, {name: "server 2", repoName: repo2, touchedServers: []*testServer{server2}, untouchedServers: []*testServer{server1}}} {
		...
		ptms := &pullthroughManifestService{ManifestService: newTestManifestService(tc.repoName, nil), imageStream: imageStream, metrics: metrics.NewNoopMetrics()}
...
func newTestManifestService(name string, data map[digest.Digest]distribution.Manifest) *testManifestService {
	...
	return &testManifestService{name: name, data: b, calls: make(map[string]int)}
}
...
func (t *testManifestService) Get(ctx context.Context, dgst digest.Digest, options ...distribution.ManifestServiceOption) (distribution.Manifest, error) {
	...
	if !exists {
		return nil, distribution.ErrManifestUnknownRevision{Name: t.name, Revision: dgst}
	}
	return content, nil
}
...
func TestPullthroughManifestMirroring(t *testing.T) {
	...
	imageStream := imagestream.New(ctx, namespace, repo, registryclient.NewFakeRegistryAPIClient(nil, imageClient))
	ms := &putWaiterManifestService{done: make(chan struct{})}
	ptms := &pullthroughManifestService{ManifestService: ms, newLocalManifestService: func(ctx context.Context) (distribution.ManifestService, error) {
		return ms, nil
	}, imageStream: imageStream, mirror: true, metrics: metrics.NewNoopMetrics()}
...
func TestPullthroughManifestMetrics(t *testing.T) {
	...
	ptms := &pullthroughManifestService{ManifestService: ms, newLocalManifestService: func(ctx context.Context) (distribution.ManifestService, error) {
		return ms, nil
	}, imageStream: imageStream, metrics: metrics.NewMetrics(sink)}
	_, err = ptms.Get(ctx, digest.Digest(img.Name))
	if err != nil {
		t.Fatalf("failed to get manifest: %v", err)
	}
	if diff := c.Diff(counter.M{fmt.Sprintf("pullthrough_repository:%s:Init", tsURL.Host): 1, fmt.Sprintf("pullthrough_repository:%s:ManifestService.Get", tsURL.Host): 1}); diff != nil {
