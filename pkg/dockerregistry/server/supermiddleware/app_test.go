...
func TestApp(t *testing.T) {
	...
	app := &testApp{log: log}
	config := &configuration.Configuration{Auth: configuration.Auth{Name: nil}, Storage: configuration.Storage{"inmemory": nil, "delete": configuration.Parameters{"enabled": true}, "cache": configuration.Parameters{"blobdescriptor": Name}}, Middleware: map[string][]configuration.Middleware{"registry": {{Name: Name}}, "repository": {{Name: Name}}, "storage": {{Name: Name}}}}
	...
	fooContent := []byte("foo")
	var location string
	serverURL := func(s string) func() string {
		return func() string {
			return server.URL + s
		}
	}
	lastLocation := func(s string) func() string {
		return func() string {
			return location + s
		}
	}
	for _, test := range []struct {
		...
		expectStatus	int
		expectLog	[]string
	}{{name: "foo_get_blob", method: "HEAD", url: serverURL("/v2/foo/blobs/" + fooDigest + "/"), expectStatus: http.StatusNotFound, expectLog: []string{"AccessController(repository::foo:pull)", "foo: enter Registry.Repository", "foo: leave Registry.Repository", "foo(regular): enter BlobStore.Stat", "foo(regular): enter BlobDescriptorService.Stat", "foo(regular): leave BlobDescriptorService.Stat", "foo(regular): leave BlobStore.Stat"}}, {name: "foo_start_blob_upload", method: "POST", url: serverURL("/v2/foo/blobs/uploads/"), expectStatus: http.StatusAccepted, expectLog: []string{"AccessController(repository::foo:pull, repository::foo:push)", "foo: enter Registry.Repository", "foo: leave Registry.Repository", "foo(regular): enter BlobStore.Create", "foo(regular): leave BlobStore.Create"}}, {name: "foo_put_blob_foo", method: "PUT", url: lastLocation("&digest=" + fooDigest), body: bytes.NewReader(fooContent), expectStatus: http.StatusCreated, expectLog: []string{"AccessController(repository::foo:pull, repository::foo:push)", "foo: enter Registry.Repository", "foo: leave Registry.Repository", "foo(regular): enter BlobStore.Resume", "foo(regular): leave BlobStore.Resume", "foo(regular): enter BlobWriter.Commit", "foo(regular): enter BlobDescriptorService.SetDescriptor", "foo(regular): leave BlobDescriptorService.SetDescriptor", "foo(regular): leave BlobWriter.Commit"}}, {name: "bar_mount_blob", method: "POST", url: serverURL("/v2/bar/blobs/uploads/?mount=" + fooDigest + "&from=foo"), expectStatus: http.StatusCreated, expectLog: []string{"AccessController(repository::bar:pull, repository::bar:push, repository::foo:pull)", "bar: enter Registry.Repository", "bar: leave Registry.Repository", "bar(regular): enter BlobStore.Create", "foo(crossmount): enter BlobDescriptorService.Stat", "foo(crossmount): leave BlobDescriptorService.Stat", "bar(regular): leave BlobStore.Create"}}, {name: "foo_delete_blob", method: "DELETE", url: serverURL("/v2/foo/blobs/" + fooDigest), expectStatus: http.StatusAccepted, expectLog: []string{"AccessController(repository::foo:delete)", "foo: enter Registry.Repository", "foo: leave Registry.Repository", "foo(regular): enter BlobStore.Delete", "foo(regular): enter BlobDescriptorService.Stat", "foo(regular): leave BlobDescriptorService.Stat", "foo(regular): enter BlobDescriptorService.Clear", "foo(regular): leave BlobDescriptorService.Clear", "foo(regular): leave BlobStore.Delete"}}} {
