...
func NewTransport(baseURL string, repoName string, creds auth.CredentialStore) (http.RoundTripper, error) {
	...
	return transport.NewTransport(nil, auth.NewAuthorizer(challengeManager, auth.NewTokenHandler(nil, creds, repoName, "pull", "push"), auth.NewBasicHandler(creds))), nil
}
...
func UploadBlob(ctx context.Context, repo distribution.Repository, desc distribution.Descriptor, content []byte) error {
	...
	uploadDesc, err := wr.Commit(ctx, distribution.Descriptor{Digest: digest.FromBytes(content)})
	if err != nil {
...
func CreateRandomTarFile() ([]byte, error) {
	...
	header := &tar.Header{Mode: 0644, ModTime: time.Now(), Typeflag: tar.TypeReg, Uname: "randocalrissian", Gname: "cloudcity", AccessTime: time.Now(), ChangeTime: time.Now()}
...
func CreateRandomImage(namespace, name string) (*imageapiv1.Image, error) {
	...
	return NewImageForManifest(fmt.Sprintf("%s/%s", namespace, name), string(manifestSchema1), "", false)
}
...
func UploadSchema2Image(ctx context.Context, repo distribution.Repository, tag string) (distribution.Manifest, error) {
	...
	cfg := map[string]interface{}{"rootfs": map[string]interface{}{"diff_ids": make([]string, len(layers))}, "history": make([]struct{}, len(layers))}
	configContent, err := json.Marshal(&cfg)
	if err != nil {
		return nil, fmt.Errorf("marshal image config: %v", err)
	}
	config := distribution.Descriptor{Digest: digest.FromBytes(configContent), Size: int64(len(configContent))}
...
func ConvertImage(image *imageapi.Image) (*imageapiv1.Image, error) {
	...
	for _, layer := range image.DockerImageLayers {
		newImage.DockerImageLayers = append(newImage.DockerImageLayers, imageapiv1.ImageLayer{Name: layer.Name, LayerSize: layer.LayerSize, MediaType: layer.MediaType})
	}
